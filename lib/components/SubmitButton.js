...
class SubmitButton extends React.Component {
  ...
  render() {
    ...
    if (this.isSubmitting()) {
      return (
        <div aria-label={isSubmittingText} className="forms-submit-button forms-submitting">{isSubmittingText}</div>
      );
    }
    else if (this.canSubmit()) {
      return (
        <div aria-label={canSubmitText} className="forms-submit-button forms-can-submit" onClick={::this.submit}>{canSubmitText}</div>
      );
    }
    else {
      return (
        <div aria-label={cannotSubmitText} className="forms-submit-button forms-cannot-submit">{cannotSubmitText}</div>
      );
    }
  }
}
