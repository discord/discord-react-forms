  ...
  render() {
    ...
    return (
      <div className="forms-checkbox">
        <input aria-labelledby={label} type="checkbox" onChange={::this.onChange} checked={field.value} name={name} ref={::this.setRef} />
        <label for={name} onClick={::this.toggleValue}>{label}</label>
      </div>
    );
  }
}
