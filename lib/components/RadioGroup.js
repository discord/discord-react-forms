    ...
    const radios = options.map((option, i) => {
      ...
      return (
        <div key={`radio-option-${i}`}>
          <input aria-labelledby={option.label} type="radio" checked={value == option.value} onChange={onChange} />
          <label for={`radio-option-${i}`} onClick={onChange}>{option.label}</label>
        </div>
      );
    });
